...
def render_horizon_strategies(strategies: List[HorizonStrategy]) -> None:
    ...
    st.write("Estrategia algorítmica para determinar cuándo entrar, liquidar o proyectar a futuro:")
...
def render_ai_hyper_forecast_box(ticker: str, metrics: MarketMetrics, assessment: QuantAssessment) -> None:
    ...
    <div class="ai-oracle-box">
        ...
        <p style="margin: 0; font-size: 0.98em; line-height: 1.6; color: #f3e8ff;">
            ...
            Nuestros cálculos indican una brecha abierta: si las políticas fiscales se consolidan, el activo buscará un techo técnico de <b>${metrics.current_price*1.28:,.2f}</b>; sin embargo, ante choques de cadena de suministro el suelo algorítmico se sitúa en <b>${metrics.current_price*0.78:,.2f}</b>.
        </p>
    </div>
    """, unsafe_allow_html=True)
...
def render_trump_prediction_dashboard(ticker: str, predictions: List[TrumpPredictionResult], current_price: float) -> None:
    ...
    for pred in predictions:
        ...
        <div style="background-color: #0f172a; border: 1px solid #1e293b; padding: 20px; border-radius: 12px; margin-bottom: 15px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <h4 style="margin: 0; color: #f8fafc;">{pred.policy_vector}</h4>
                <span style="color: {color_badge}; background: {bg_badge}; padding: 4px 10px; border-radius: 6px; font-size: 0.8em; font-weight: 700;">{pred.sentiment_label}</span>
            </div>
            <p style="margin: 12px 0; color: #cbd5e1; font-size: 0.95em;">{pred.analysis_justification}</p>
        </div>
        """, unsafe_allow_html=True)

    avg_score = total_score / len(predictions) if predictions else 0.0
    st.markdown("---")
    st.markdown("### 🧮 Calculadora de Posición Avanzada")
    capital_invertido = st.number_input("Monto total asignado a la posición ($ USD):", min_value=10.0, value=10000.0, step=500.0)
    acciones_compradas = capital_invertido / current_price
    st.info(f"💼 Volumen Real Adquirido: **{acciones_compradas:,.4f}** unidades de {ticker}.")
